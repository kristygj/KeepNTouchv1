...
from app.forms import Formname, formCreateEvent, LoginForm, confirmParticipation, deleteParticipation, UploadForm, UpdateAccount, EmptyForm, EditProfileForm, LeaveFeedback,FeedbackForm
...
def home():
    ...
    if "submitE" in request.form and LeaveFeedback.validate_on_submit():
       ...
       event = Event.query.filter_by(id=event_id).first()
       student = Student.query.filter_by(id=session.get('id')).first()
       return render_template('feedback_post.html', event=event, student=student, forms=forms, LeaveFeedback=LeaveFeedback, feedback=feedback, form=form)

    if not Student.query.join(partecipation).join(Event).filter( ).first():#& (feedback.c.student_id == event.id)).first():
           # event.numofPeople += 1
    ...
    if current_user.is_authenticated:
        ...
        #return render_template('index.html', events=events, confirmParticipation=confirmParticipation, deleteParticipation=deleteParticipation, partecipation=partecipation, Student = Student, Event=Event, BPs = BusinessPartner, and_=and_)
         if Student.query.filter_by(email=current_user.email).first():  ## if student then show the wall
            ...
            return render_template('index.html', events=events, confirmParticipation=confirmParticipation, deleteParticipation=deleteParticipation, partecipation=partecipation, Student = Student, Event=Event, BPs = BusinessPartner, and_=and_, feedback=feedback, LeaveFeedback=LeaveFeedback, form=form)
         else :
...
def profile():
    ...
    if current_user.is_authenticated:
        ...
        return render_template('profile.html',student=st, form=form, image_file=image_file, eform=eform, posts=posts)
    else:
        return redirect('login')
...
def feedback():
    ...
    if form.validate_on_submit():
       ...
       return redirect(url_for('home'))
    return render_template('feedback_post.html', title='Feedback', form = form)
